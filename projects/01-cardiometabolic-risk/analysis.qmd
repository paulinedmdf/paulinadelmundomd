...
**Use a weighted estimator whenever a sample was drawn with unequal probabilities — NHANES, BRFSS, MEPS, NHIS, NHATS, all the federal surveys.** The unweighted version isn't "simpler"; it's biased.
:::
...
Every statistical test or estimator used above, with a one-line definition, what it tests, when you'd use it, the key assumption, and what to do if that assumption fails.

| Method | What it does | Why used here | Key assumption | If assumption fails |
|---|---|---|---|---|
| **Skewness, excess kurtosis** (`e1071::skewness/kurtosis`) | Two numbers describing the shape of a distribution. Skew measures asymmetry (right-skew = long upper tail); excess kurtosis measures tail-heaviness vs a normal. | Decide whether a continuous variable is symmetric enough for normal-theory methods, or right-skewed enough to need log/transformation. | None — descriptive. | N/A |
| **Distribution fit by AIC** (`fitdistrplus::fitdist`) | Fits candidate distributions (normal, log-normal, gamma) by maximum likelihood; AIC = 2k − 2logL ranks them. Lower AIC = better fit penalizing complexity. | Pick the right regression family / link function for any variable used as outcome (e.g., triglycerides → log-normal). | Maximum likelihood requires correctly-specified support (positive values only for log-normal/gamma). | Plot empirical vs theoretical CDF and pick visually. |
| **Welch's t-test** (`stats::t.test`) | Tests whether two group means differ, allowing unequal variances. Reports a t statistic, df (Satterthwaite-adjusted), and a p-value. | Compare a continuous variable (age, BMI) between two groups (included vs excluded). | Roughly normal distribution within each group, or large enough n for the central-limit theorem to kick in. | Use Mann-Whitney U / Wilcoxon rank-sum (non-parametric). |
| **Pearson chi-squared** (`stats::chisq.test`) | Tests independence between two categorical variables by comparing observed vs expected cell counts under independence. | Compare a categorical variable's distribution between two groups (sex × included; race × included). | Expected cell counts ≥ 5 in most cells. | Use Fisher's exact test (exact small-sample). |
| **Mann-Whitney U / Wilcoxon rank-sum** (`stats::wilcox.test`) | Non-parametric test that the distribution of values in group A is shifted relative to group B. Operates on ranks, not raw values. | Compare PCE risk between MetS+ and MetS-; compare beneficiary counts between high-z and moderate-z groups. | Independent observations; no distributional assumption. | Permutation test if independence violated. |
| **Pearson correlation** (`stats::cor`, method = "pearson") | Measures the strength of *linear* association between two continuous variables; r ranges −1 to +1. | Detect collinear predictors (e.g., BMI vs waist) before regression — convention is |r| ≥ 0.7 means don't use both. | Roughly linear relationship; no extreme outliers. | Use Spearman rank correlation (monotonic but non-linear OK). |
| **Variance Inflation Factor (VIF)** | For each predictor, regress it on all the others; VIF = 1 / (1 − R²). Tells you how much the SE of that predictor's coefficient is inflated by collinearity. | Quantify collinearity in the fitted regression specification. | Linear regression for the auxiliary fit. | If high VIF, drop the redundant predictor or use ridge/lasso. |
| **Random-intercept (mixed) model** (`lme4::glmer`) | A regression that adds random intercepts at each clustering level (here: stratum, PSU). Captures the fact that observations within a cluster are more similar than across clusters. | Quantify the share of variance at each NHANES sampling level — the **ICC**. | Random-effect distribution is approximately normal. | Use a marginal/GEE model instead. |
| **Intraclass Correlation Coefficient (ICC)** | The fraction of total outcome variance that lies *between* clusters rather than within. ICC = 0 means clustering doesn't matter; ICC closer to 1 means observations within a cluster are very similar. | Justify why design-aware estimators (svyglm) matter — when ICC at PSU/stratum is non-trivial, ignoring it inflates Type-I error. | Outcome variance can be partitioned; for binary outcomes the residual is fixed at π²/3 on the latent (logit) scale. | N/A — descriptive variance partition. |
| **Survey-weighted mean / regression** (`survey::svymean`, `survey::svyglm`) | Same as the unweighted versions, but each row contributes proportionally to its sampling weight; SEs use Taylor linearization to account for the strata + PSU + weights. | Estimate population prevalence and odds ratios that generalize to the US adult population, not just the sampled adults. | Sampling weights and design variables are correctly specified. | Use a Bayesian model with informative priors, or post-stratification on a known population. |
| **Logistic regression** | Models the log-odds of a binary outcome as a linear combination of predictors. Coefficients exponentiated are odds ratios. | The MetS outcome is binary; linear regression gives nonsensical predictions outside [0, 1]. | The log-odds are linear in the predictors (no need for the outcome to be linearly related to predictors). | Use splines on continuous predictors, or a non-linear classifier (XGBoost). |
| **MICE — Multiple Imputation by Chained Equations** (`mice::mice`) | Generates m plausible completions of the dataset by imputing each missing variable conditional on the others. Predictive mean matching is the default for continuous variables. | Recover the rows lost to fasting-subsample dropout without biasing toward the morning subsample. | Missing-at-random (MAR) — missingness depends on observed but not unobserved variables. | Use a more flexible imputation method or Bayesian model that explicitly models missingness mechanism. |
| **Rubin's rules pooling** (`mitools::MIcombine`) | Combines analysis results across the m imputations: pooled estimate = average; pooled variance = within-imputation variance + (1 + 1/m) × between-imputation variance. | Get one combined coefficient + honest CI from the m parallel svyglm fits. | The analysis estimator is (approximately) normally distributed within each imputation. | Use bootstrap-based pooling. |
| **Pearson chi-squared, AIC, t-test → all of the above also live in this primer** because the same tests recur in different sections — see rows above. | | | | |
| **Gradient-boosted trees (XGBoost)** | A non-linear classifier built by sequentially adding decision trees that correct the errors of the previous ones. No assumption of linearity or distribution shape. | A flexible benchmark for what demographics + BMI alone can predict; complements the design-aware logit. | Enough training data to avoid overfitting; representative train/test split. | Use a simpler model (logistic) with engineered features; report cross-validated metrics. |
| **AUC (ROC-AUC)** | The probability that a randomly chosen positive case is ranked higher than a randomly chosen negative case. Pure ranking metric. | Quantify discrimination — how well the model separates MetS+ from MetS-. | Ranking interpretation is invariant to rescaling of probabilities. | Use precision-recall AUC for very imbalanced outcomes. |
| **Brier score** | Mean squared error between predicted probability and the binary outcome. Penalizes both bad ranking and miscalibration. | A single-number summary that captures both calibration and discrimination. | Probabilities are interpretable (not just ranks). | If only ranks matter, use AUC. |
| **Calibration curve** | Bins predictions into deciles, plots observed frequency vs predicted probability per bin. Perfectly calibrated = on the diagonal. | Test whether predicted probabilities are usable as risk numbers, not just rankings. | Enough cases per bin (n ≥ 30) for stable observed frequencies. | Use Brier or Expected Calibration Error. |
...
| **Pooled Cohort Equations (PCE)** | The 2013 ACC/AHA-recommended 10-year ASCVD risk score with race × sex specific coefficients (Goff et al. 2014). | Provide an established clinical comparator instead of evaluating the boosted model in a vacuum. | Inputs are within the score's supported ranges (age 40–79, NH White or Black). | Use the 2023 PREVENT score (drops the race term). |
