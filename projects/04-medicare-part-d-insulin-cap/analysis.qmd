...
The Wald test answers the joint question "are these $q$ coefficients all simultaneously zero?" rather than testing one at a time. Encode the restrictions as $\mathbf{R}\beta = \mathbf{r}$ (with $\mathbf{R}$ a $q \times p$ selector matrix); the Wald statistic is
$$W = (\mathbf{R}\hat\beta - \mathbf{r})^\top \!\left[\mathbf{R}\, \widehat{\text{Var}}(\hat\beta)\, \mathbf{R}^\top\right]^{-1} \!(\mathbf{R}\hat\beta - \mathbf{r}),$$
and the F-version is $F = W/q \sim F_{q, n-p}$ under the null. The middle term is a Mahalanobis-style metric: it measures how far $\hat\beta$ is from satisfying the restrictions in units that account for the coefficients' own sampling variance and correlations. The joint test is the right tool because individually, pre-period coefficients can each be near zero by chance even when they share a common drift; the joint test detects whether the *collection* has a zero center, robust to single-coefficient noise and sensitive to coordinated deviation. **A non-rejection isn't proof of parallel trends**, only failure to falsify. Pre-period leads usually have wide SEs (year-by-treatment interactions on small subsamples), and with only 2–3 pre-period years the test cannot detect drift smaller than ~0.10 — which would still bias the post-period coefficient. The event-study plot and the F-test together are the test; either alone is a trap. Concrete example: three pre-period coefficient estimates $(0.02, -0.01, 0.04)$ with an SE-and-correlation structure producing $W \approx 2.6$ and $F \approx 0.87$ under $F_{3, \infty}$ gives $p \approx 0.46$ — the parallel-trends null is *not* rejected, the data are consistent with what the event-study plot suggested.
:::
