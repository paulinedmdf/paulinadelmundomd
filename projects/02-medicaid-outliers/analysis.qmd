...
Use robust z whenever your data have heavy tails (claims data, gene expression, financial returns, anything log-skewed). Use the classical z when you've genuinely confirmed normality (rare in real-world health data).
:::
...
Use BH whenever you're testing many hypotheses simultaneously — genomics, neuroimaging, fraud detection, A/B-testing dashboards. **Bonferroni** (divide α by m) is more conservative; use Bonferroni when you need a strict family-wise error rate guarantee, BH when controlling the false discovery proportion is enough.
:::
...
Use Gini for any concentration analysis: spending across counties, hospitalizations across providers, citations across papers, wealth across households. **Don't use it for symmetric-around-zero quantities** (it's defined on non-negative values).
:::
...
Every statistical test, estimator, or transformation used above, with a one-line definition, what it does here, the key assumption, and what to do if that assumption fails.

| Method | What it does | Why used here | Key assumption | If assumption fails |
|---|---|---|---|---|
| **Median, MAD, robust z** | Median = middle value (robust to outliers); MAD = median absolute deviation from the median; robust z = (x − median) / (1.4826 × MAD), interpretable as "approximate SDs from the median" under normality. | Per-HCPCS peer-comparison flagging — heavy-tailed claims data make mean/SD z's worthless. | Errors are roughly symmetric around the median; 1.4826 calibration is exact only for normal data. | Use bootstrap-derived peer percentiles instead. |
| **Variance-to-mean ratio (VMR)** | The ratio var(x) / mean(x). For a Poisson distribution VMR = 1; values above 1 indicate **overdispersion** (more spread than Poisson can explain). | Diagnose whether count-like features (paid, beneficiaries, claims) need negative binomial or ZINB instead of plain Poisson in any future modeling. | Variable is non-negative count-like. | None — descriptive. |
| **% zeros / zero-inflation count** | Fraction of observations equal to zero. | Flag when a count distribution has more zeros than even an overdispersed NB can produce → use **zero-inflated** models. | Zeros are meaningfully distinct from small positive counts (structural zeros). | Use a hurdle model. |
| **Pearson correlation** | Linear association between two continuous variables; r ∈ [−1, +1]. | Detect collinear features before fitting the isolation forest (e.g., claims vs beneficiaries are nearly the same construct). | Linear relationship; no extreme outliers. | Use Spearman rank correlation. |
| **Condition number (κ)** | Ratio of largest to smallest singular value of the standardized design matrix. κ > 30 = problematic collinearity. | Single-number summary of multicollinearity across all features at once — complements the pairwise correlation matrix. | Standardized features (mean 0, var 1). | Drop redundant features or use ridge regression. |
| **Variance Inflation Factor (VIF)** | For each feature, regress it on the others; VIF = 1 / (1 − R²). High VIF = the feature is mostly explained by the others. | Per-feature collinearity diagnostic for the isolation-forest input set. | Linear regression for the auxiliary fit. | Drop the redundant feature. |
| **Two-sided p-value from a normal approximation** | Convert a z-score to a probability by `2 × Φ(−|z|)` where Φ is the normal CDF. | Step needed before applying BH-FDR — turn each robust z into a tail probability. | The null distribution of the robust z is approximately normal. | Use a permutation-derived empirical null. |
| **Benjamini–Hochberg FDR** (`p.adjust(method = "BH")`) | Adjusts a vector of p-values to control the false discovery rate (the expected fraction of false positives among rejections). | Required when testing thousands of NPI × HCPCS rows simultaneously — a hard z ≥ 5 cutoff with no multiplicity control will produce many chance flags. | Independent or positively-dependent tests. | Use Benjamini–Yekutieli (works under arbitrary dependence). |
| **Mann-Whitney U / Wilcoxon rank-sum** (`stats::wilcox.test`) | Non-parametric test that the distribution of values in group A is shifted relative to group B. Operates on ranks. | Compare beneficiary counts and paid amounts between high-z and moderate-z subsets — robust to the heavy tails. | Independent observations; no distributional assumption. | Permutation test. |
| **Pearson chi-squared** (`stats::chisq.test`) | Tests independence between two categorical variables. | Compare specialty mix and state distribution between high-z and moderate-z flagged rows. | Expected cell counts ≥ 5 in most cells. | Use Fisher's exact test. |
| **Isolation forest** | Unsupervised anomaly detector that builds many random binary trees and scores points by how easily they get isolated (shorter average path = more anomalous). | Second-opinion flagging on the joint distribution of features — catches multivariate anomalies the per-row z misses. | Anomalies are "few and different." | Use one-class SVM, LOF, or an autoencoder. |
| **NTILE / decile binning** (`dplyr::ntile`) | Splits a vector into n equal-sized rank bins (deciles = 10 bins of equal n). | Within-category ranking for the choropleth — coloring is comparable inside a category, not across them. | Continuous variable with few ties. | Use `cut()` on quantiles to allow unequal bins. |
| **Gini coefficient** | Twice the area between the Lorenz curve and the diagonal of perfect equality. 0 = uniform; 1 = one unit has everything. | Quantify spatial concentration of spend within each Medicaid category. | Non-negative quantity; no missing values. | Report top-K share or Theil index. |
| **Top-K share** | Sum of the K largest values divided by the total. | Companion to Gini for spatial concentration; says concretely "the top 10 counties carry X% of the category's spend." | Ordering by the same metric you're summing. | None — fully transparent. |
| **`patchwork::wrap_plots`** | Composes multiple ggplot objects into one figure with shared layout. | Pack four diagnostic histograms into a single 2×2 panel. | All inputs are ggplot objects. | Use `gridExtra::grid.arrange` or base layouts. |
| **Provider taxonomy joins (NPPES + NUCC + ZCTA → county)** | Sequence of left joins to enrich each NPI with state, specialty, and county-FIPS via NPPES → NUCC taxonomy and Census ZCTA crosswalks. | Without this enrichment the flagged outliers are just NPIs; with it they're "an internist in Hennepin County, MN." | Crosswalk tables match correctly on join keys. | Report the % of rows that fail the join (geo-coverage diagnostic). |
