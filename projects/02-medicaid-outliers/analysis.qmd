...
## Why this analysis, and what you'll learn from it
...
::: {.callout-note}
## A note on terminology — the acronyms used in this notebook
...
The raw files stay local on disk (11 GB Medicaid + 45 GB NPPES, far too large to commit). The [`prep.py`](prep.py) script runs one DuckDB pipeline that produces seven committed Parquet artifacts under `data/processed/`; this notebook reads only those.
:::
...
## Step 1 — what does the dataset actually contain?
...
## Step 3 — peer-group outlier surfacing (exploratory)
...
## Step 4 — a second opinion: unsupervised anomaly detection
...
The robust-z method asks one question per row: *is this row's paid-per-beneficiary unusually far from the peer median for its billing code?* That's a one-dimensional check — it only looks at one variable at a time. A row could look perfectly typical on paid-per-beneficiary alone but still be unusual in the *joint* pattern of (paid, beneficiaries, claims, months-active) considered together.
...
### Feature collinearity: a brief check before fitting
...
## Step 5 — where does the dollar go? A geographic atlas
